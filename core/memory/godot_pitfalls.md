...
### T-002 桌面自动化验收前先确认窗口尺寸与输入焦点
- 现象：Godot 桌面游戏在默认窗口尺寸下，世界地图右侧或下方的新入口不一定完整可见；Computer Use / Apple event 注入的 WASD 也可能没有像人工键盘一样驱动角色移动，但鼠标点击可见 Control 入口仍可生效。
- 根因：自动化工具看到的是当前 OS 窗口实际视口，不等同于 VAT 的固定 viewport；Godot 输入焦点、窗口缩放、Control 捕获和 OS 级按键注入路径都会影响真实桌面体验。
- 解决：
  - 做真实窗口 QA 前先通过窗口 zoom/fullscreen 或固定启动分辨率扩大视口，再操作远端设施或边缘 UI。
  - 同一入口尽量同时验证「直接点击可见设施」和「键盘靠近后按交互键」两种路径；如果键盘注入异常，单独记录为桌面输入兼容问题，不要用它否定 UI 点击路径。
  - 保留 VAT/Smoke 的固定视口测试作为回归基线，桌面自动化只补真实窗口尺寸、焦点和可发现性风险。
- 影响范围：所有需要 AI 桌面自动化测试 Godot 4.x 桌面游戏的项目。
- 录入：2026-05 / Game_RogueCard。
- 复现频次：+1
